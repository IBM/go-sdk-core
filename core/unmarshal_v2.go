...
func UnmarshalPrimitive(rawInput map[string]json.RawMessage, propertyName string, result interface{}) (err error) {
	if propertyName == "" {
		err = SDKErrorf(nil, errorPropertyNameMissing, "no-prop-name", getSystemInfo)
	}
	...
	if foundIt && rawMsg != nil {
		...
		if err != nil {
			...
			err = SDKErrorf(nil, errMsg, "json-unmarshal-error", getSystemInfo)
		}
	}
	return
}
...
func UnmarshalModel(rawInput interface{}, propertyName string, result interface{}, unmarshaller ModelUnmarshaller) (err error) {
	...
	if IsNil(rawInput) {
		err = SDKErrorf(nil, errorUnmarshallInputIsNil, "no-input", getSystemInfo)
		return
	}
	...
	case reflect.Slice:
		...
		case reflect.Slice:
			...
			default:
				...
				err = SDKErrorf(nil, errMsg, "bad-slice-elem-slice-type", getSystemInfo)
			}

		default:
			errMsg := fmt.Sprintf(errorUnsupportedResultType, rResultType.String())
			err = SDKErrorf(nil, errMsg, "bad-slice-elem-type", getSystemInfo)
			return
		}
	...
	case reflect.Map:
		...
		case reflect.Slice:
			...
			default:
				errMsg := fmt.Sprintf(errorUnsupportedResultType, rResultType.String())
				err = SDKErrorf(nil, errMsg, "bad-slice-elem-map-type", getSystemInfo)
				return
			}
		default:
			errMsg := fmt.Sprintf(errorUnsupportedResultType, rResultType.String())
			err = SDKErrorf(nil, errMsg, "bad-map-entry-type", getSystemInfo)
			return
		}

	default:
		errMsg := fmt.Sprintf(errorUnsupportedResultType, rResultType.String())
		err = SDKErrorf(nil, errMsg, "bad-model-type", getSystemInfo)
		return
	}

	err = RepurposeSDKError(err, "unmarshal-fail")
...
func unmarshalModelInstance(rawInput interface{}, propertyName string, result interface{}, unmarshaller ModelUnmarshaller) (err error) {
	...
	if err != nil {
		...
		err = SDKErrorf(nil, errMsg, "input-source-error", getSystemInfo)
		return
	}
	...
	if foundInput && rawMap != nil {
		...
		if err != nil {
			errMsg := fmt.Sprintf(errorUnmarshalModel, propInsert(propertyName), getModelResultType(result), err.Error())
			err = SDKErrorf(nil, errMsg, "unmarshaller-error", getSystemInfo)
			return
		}
	}
	return
...
func unmarshalModelSlice(rawInput interface{}, propertyName string, result interface{}, unmarshaller ModelUnmarshaller) (err error) {
	...
	if sliceSize > 0 {
		...
		if err != nil {
			errMsg := fmt.Sprintf(errorUnmarshalModel, propInsert(propertyName),
				reflect.TypeOf(result).Elem().String(), err.Error())
			err = SDKErrorf(nil, errMsg, "result-type-error", getSystemInfo)
			return
		}
...
func getUnmarshalInputSourceMap(rawInput interface{}, propertyName string) (foundInput bool, inputSource map[string]json.RawMessage, err error) {
	...
	if !ok {
		...
		err = SDKErrorf(nil, errMsg, "non-json-source-map", getSystemInfo)
		return
	}
	...
	if propertyName != "" {
		...
		if !foundInput || isJsonNull(rawMsg) {
			...
			if err != nil {
				foundInput = false
				err = SDKErrorf(nil, err.Error(), "json-unmarshal-error", getSystemInfo)
				return
			}
		}
	}
...
func getUnmarshalInputSourceSlice(rawInput interface{}, propertyName string) (foundInput bool, inputSource []json.RawMessage, err error) {
	...
	if propertyName != "" {
		...
		if !ok {
			errMsg := fmt.Sprintf(errorIncorrectInputType, "map[string]json.RawMessage", reflect.TypeOf(rawInput).String())
			err = SDKErrorf(nil, errMsg, "raw-input-error", getSystemInfo)
			return
		}
		...
		if !ok {
			...
			err = SDKErrorf(nil, errMsg, "no-name-raw-input-error", getSystemInfo)
			return
		}
...
func getUnmarshalResultType(result interface{}) (ptrType reflect.Type, err error) {
	...
	default:
		errMsg := fmt.Sprintf(errorUnsupportedResultType, rResultType.String())
		err = SDKErrorf(nil, errMsg, "unsupported-type", getSystemInfo)
	}
	return
}
