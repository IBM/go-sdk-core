...
func (this BasicAuthenticator) Validate() error {
	if this.Username == "" {
		return SDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_MISSING, "Username"), "no-user", getSystemInfo)
	}

	if this.Password == "" {
		return SDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_MISSING, "Password"), "no-pass", getSystemInfo)
	}

	if HasBadFirstOrLastChar(this.Username) {
		return SDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_INVALID, "Username"), "bad-user", getSystemInfo)
	}

	if HasBadFirstOrLastChar(this.Password) {
		return SDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_INVALID, "Password"), "bad-pass", getSystemInfo)
	}
