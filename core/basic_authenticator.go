...
func NewBasicAuthenticator(username string, password string) (*BasicAuthenticator, error) {
	...
	if err := obj.Validate(); err != nil {
		err = rewrapSDKError(err, "NewBasicAuthenticator")
		return nil, err
	}
	return obj, nil
}
...
func newBasicAuthenticatorFromMap(properties map[string]string) (*BasicAuthenticator, error) {
	if properties == nil {
		return nil, coreSDKErrorf(nil, ERRORMSG_PROPS_MAP_NIL, "missing_props", "newBasicAuthenticatorFromMap")
	}
...
func (this BasicAuthenticator) Validate() error {
	if this.Username == "" {
		return coreSDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_MISSING, "Username"), "no-user", "BasicAuthenticator.Validate")
	}

	if this.Password == "" {
		return coreSDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_MISSING, "Password"), "no-pass", "BasicAuthenticator.Validate")
	}

	if HasBadFirstOrLastChar(this.Username) {
		return coreSDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_INVALID, "Username"), "bad-user", "BasicAuthenticator.Validate")
	}

	if HasBadFirstOrLastChar(this.Password) {
		return coreSDKErrorf(nil, fmt.Sprintf(ERRORMSG_PROP_INVALID, "Password"), "bad-pass", "BasicAuthenticator.Validate")
	}
